"""원화를 **정확히 N칸 격자** 위로 다시 찍는다.

🔴 **2026-09-13 — 캐릭터는 더 이상 이것을 거치지 않는다** (검토 41 승인).
양자화가 세부를 6.2배 깎고 있었다. 캐릭터는 `bake_art.py` 가 실루엣을 잘라
발바닥에 앉히고 줄이는 것으로 끝난다. **이 도구는 배경 전용으로 남는다.** (`reports/32_원화v3_검수.md` 3절)
